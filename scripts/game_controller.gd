...
## The old main.tscn HUD labels are superseded by the procedural HUD.
func _hide_legacy_ui() -> void:
	for n in ["TitleLabel", "StatusLabel", "DepthLabel", "DepthBarBg",
			"DepthBarFill", "CenterMessage", "PauseLabel"]:
		var node := _canvas.get_node_or_null(n)
		if node:
			node.visible = false
