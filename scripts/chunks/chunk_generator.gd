...
## Builds one vertical procedural level out of authored chunk scenes.
##
## High-level flow each time build_level() is called:
##   1. Resolve the LevelConfig (start chunk, finale chunk, middle pool,
##      and tuning knobs). See scripts/chunks/level_config.gd.
##   2. Pick a sequence of middle chunks via weighted random selection while
##      respecting a per-level difficulty budget, so levels stay solvable.
##   3. Bookend the sequence with the start chunk (safe spawn) and the
##      finale chunk (green exit gate).
##   4. Instantiate each chunk and align it vertically by snapping the new
##      chunk's ConnectTop marker to the previous chunk's ConnectBottom.
##   5. Hand control back to GameController with spawn / bottom / sequence
##      info so it can place the ball and the camera bounds.
##
...
##
## To change the chunk lineup or the difficulty curve, EDIT THE LEVEL
## CONFIG RESOURCE (resources/levels/default_level_config.tres). You do
## not need to edit this script for normal level-design work.

## The level recipe (chunk pool, start/finale, length, difficulty).
## Defaults to resources/levels/default_level_config.tres.
@export var config: LevelConfig
## Optional biome filter. Empty = include every chunk regardless of its
## biome tag. Set e.g. &"factory" to restrict to chunks marked for that
## biome (chunks with no biome set always pass, so they act as universal).
@export var active_biome: StringName = &""

# Fallback pool used when the config does not specify one. Kept short and
# guaranteed solvable so the game still boots even if the config is empty.
const _FALLBACK_POOL_PATHS := [
	"res://resources/chunks/chunk_def_flip_std.tres",
	"res://resources/chunks/chunk_def_flip_drop.tres",
...
# fallbacks have been applied. Populated in _resolve_config_defaults().
var _start_chunk: ChunkDefinition
var _finale_chunk: ChunkDefinition
var _chunk_pool: Array[ChunkDefinition] = []


func _resolve_config_defaults() -> void:
	if config == null:
		config = load(_DEFAULT_CONFIG_PATH) as LevelConfig
	_start_chunk = config.start_chunk if is_instance_valid(config.start_chunk) \
		else load(_DEFAULT_START_PATH) as ChunkDefinition
	_finale_chunk = config.finale_chunk if is_instance_valid(config.finale_chunk) \
		else load(_DEFAULT_FINALE_PATH) as ChunkDefinition
	_chunk_pool = []
	for c in config.chunk_pool:
		if is_instance_valid(c):
			_chunk_pool.append(c)
	if _chunk_pool.is_empty():
		for path in _FALLBACK_POOL_PATHS:
			var def := load(path) as ChunkDefinition
			if is_instance_valid(def):
				_chunk_pool.append(def)
...
func build_level(world: Node2D, rng: RandomNumberGenerator, level_index: int = 1) -> Dictionary:
	...
	if not is_instance_valid(config):
		push_error("ChunkGenerator: missing LevelConfig")
		return {}
	if _chunk_pool.is_empty() or not is_instance_valid(_start_chunk) or not is_instance_valid(_finale_chunk):
	...
	var biome_pool := _filter_biome(_chunk_pool)
	...
	sequence.append(_start_chunk)
	remaining_budget -= _start_chunk.difficulty
	used_budget += _start_chunk.difficulty
	...
	if _finale_chunk.difficulty > remaining_budget:
		push_warning("ChunkGenerator: mandatory finale chunk exceeds difficulty budget")
	sequence.append(_finale_chunk)
	remaining_budget -= _finale_chunk.difficulty
	used_budget += _finale_chunk.difficulty
	...
	# Scale moving-hazard speed for this level ("faster hazards"). Done
	# after all chunks are built so it only touches this freshly generated
	# level and never a queued-free old one.
...
## Snaps a chunk's screen-edge side walls (WallLeft / WallRight sitting
## near the arena edges) to one canonical width and inner face. Adjacent
## chunks then form a single flush column with no ledge to snag the ball.
## Walls deliberately placed far inward (tight chunks like the gauntlet)
## sit well away from the edge and are intentionally left untouched.
...
## keeping its authored height. A fresh shape is used so shared
## SubResources are never mutated.
...
## Recursively multiplies the `speed` of every moving_hazard mover under
## the given node. Scoped to the freshly built `world` subtree so it never
